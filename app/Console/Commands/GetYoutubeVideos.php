    ...
    private function __getUploadsFromUploadsPlaylist($playlistId, $channel)
    {
      ...
      while ($continue) {
        ...
        foreach ($findUploadsList->getItems() as $item) {
          ...
          // check eligible
          if (preg_match('/kibblelands/im', $data['title'])) // need contains kibblelands in title
            if (preg_match('/kibblelands/im', $data['description'])) // need contains kibblelands in description
              if (preg_match('/kibblelands\.(net)/im', $data['description'])) // need contains link to kibblelands.net
